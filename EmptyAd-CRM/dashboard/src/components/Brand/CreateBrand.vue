...
const brandItem = ref<BrandCreateModel>({
  name: '',
  url: '',
  image: null as any,
})
...
function handleImageUpload(options: {
  ...
  console.log('Brand image upload options:', options)
  if (options.fileList && options.fileList.length > 0) {
    brandItem.value.image = options.fileList[0].file
    console.log('Brand image set:', brandItem.value.image)
  }
}

function handleImageRemove() {
  brandItem.value.image = null
}
</script>
